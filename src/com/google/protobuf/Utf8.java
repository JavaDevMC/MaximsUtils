...
final class Utf8 {
    ...
    final String decodeUtf8Default(ByteBuffer buffer, int index, int size) throws InvalidProtocolBufferException {
      ...
      while (offset < limit) {
        ...
        if (DecodeUtil.isTwoBytes(byte1)) {
          ...
          DecodeUtil.handleTwoBytes(byte1, buffer
        ...
        if (DecodeUtil.isThreeBytes(byte1)) {
          if (offset >= limit - 1)
            throw InvalidProtocolBufferException.invalidUtf8(); 
          DecodeUtil.handleThreeBytes(byte1, buffer
        ...
        if (offset >= limit - 2)
          throw InvalidProtocolBufferException.invalidUtf8(); 
        DecodeUtil.handleFourBytes(byte1, buffer
  ...
  static final class UnsafeProcessor extends Processor {
    ...
    String decodeUtf8(byte[] bytes, int index, int size) throws InvalidProtocolBufferException {
      ...
      while (offset < limit) {
        ...
        if (DecodeUtil.isTwoBytes(byte1)) {
          if (offset >= limit)
            throw InvalidProtocolBufferException.invalidUtf8(); 
          DecodeUtil.handleTwoBytes(byte1,
              UnsafeUtil.getByte(bytes, offset++), resultArr, resultPos++);
          continue;
        } 
        if (DecodeUtil.isThreeBytes(byte1)) {
          if (offset >= limit - 1)
    ...
    private static int unsafeEstimateConsecutiveAscii(long address, int maxChars) {
      ...
      if (remaining < 16)
        return 0; 
      int unaligned = 8 - ((int)address & 0x7);
...
/* Location:              C:\Users\Win10\Desktop\VoxilityCraftServer\Bungee\BungeeCord.jar!\com\google\protobuf\Utf8.class
