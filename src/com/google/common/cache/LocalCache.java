...
class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
  ...
  enum Strength {
    ...
    SOFT {
      <K, V> ValueReference<K, V> referenceValue(Segment<K, V> segment, ReferenceEntry<K, V> entry, V value, int weight) {
        return (weight == 1) ? 
          ...
          new WeightedSoftValueReference<>(segment.valueReferenceQueue, value, entry, weight);
      }
    ...
    WEAK {
      <K, V> ValueReference<K, V> referenceValue(Segment<K, V> segment, ReferenceEntry<K, V> entry, V value, int weight) {
        return (weight == 1) ? 
          ...
          new WeightedWeakValueReference<>(segment.valueReferenceQueue, value, entry, weight);
      }
    ...
    abstract <K, V> ValueReference<K, V> referenceValue(Segment<K, V> param1Segment, ReferenceEntry<K, V> param1ReferenceEntry, V param1V, int param1Int);
  ...
  enum EntryFactory {
    ...
    abstract <K, V> ReferenceEntry<K, V> newEntry(Segment<K, V> param1Segment, K param1K, int param1Int, ReferenceEntry<K, V> param1ReferenceEntry);
  }
  ...
  static class Segment<K, V> extends ReentrantLock {
    ...
    Segment(LocalCache<K, V> map, int initialCapacity, long maxSegmentWeight, AbstractCache.StatsCounter statsCounter) {
      ...
      this
        ...
        .writeQueue = map.usesWriteQueue() ? new WriteQueue<>() : LocalCache.<ReferenceEntry<K, V>>discardingQueue();
      this
        
        .accessQueue = map.usesAccessQueue() ? new AccessQueue<>() : LocalCache.<ReferenceEntry<K, V>>discardingQueue();
    }
    ...
    V waitForLoadingValue(ReferenceEntry<K, V> e, K key, ValueReference<K, V> valueReference) throws ExecutionException {
      ...
      try {
        ...
        if (value == null) {
          ...
          throw new CacheLoader.InvalidCacheLoadException((new StringBuilder(35 + String.valueOf(str).length())).append("CacheLoader returned null for key ").append(str).append(".").toString());
        } 
        long now = this.map.ticker.read();
        recordRead(e, now);
        return value;
        ...
        for (e = first; e != null; e = e.getNext()) {
          ...
          if (e.getHash() == hash && entryKey != null && this.map.keyEquivalence
            ...
            if (valueReference.isLoading() || (checkTime && now - e
              ...
              return null; 
            this.modCount++;
            LoadingValueReference<K, V> loadingValueReference1 = new LoadingValueReference<>(valueReference);
      ...
      if (!keysToLoad.isEmpty())
        try {
          ...
          for (K key : keysToLoad) {
            ...
            if (value == null) {
              String str = String.valueOf(key);
              throw new CacheLoader.InvalidCacheLoadException((new StringBuilder(37 + String.valueOf(str).length())).append("loadAll failed to return a value for ").append(str).toString());
            } 
            result.put(key, value);
          } 
  ...
  Map<K, V> loadAll(Set<? extends K> keys, CacheLoader<? super K, V> loader) throws ExecutionException {
    ...
    if (result == null) {
      this.globalStatsCounter.recordLoadException(stopwatch.elapsed(TimeUnit.NANOSECONDS));
      String str = String.valueOf(loader);
      throw new CacheLoader.InvalidCacheLoadException((new StringBuilder(31 + String.valueOf(str).length())).append(str).append(" returned null map from loadAll").toString());
    } 
    stopwatch.stop();
    ...
    if (nullsPresent) {
      this.globalStatsCounter.recordLoadException(stopwatch.elapsed(TimeUnit.NANOSECONDS));
      String str = String.valueOf(loader);
      throw new CacheLoader.InvalidCacheLoadException((new StringBuilder(42 + String.valueOf(str).length())).append(str).append(" returned null keys or values from loadAll").toString());
    } 
...
/* Location:              C:\Users\Win10\Desktop\VoxilityCraftServer\Bungee\BungeeCord.jar!\com\google\common\cache\LocalCache.class
