...
final class JdkBackedImmutableBiMap<K, V> extends ImmutableBiMap<K, V> {
  ...
  static <K, V> ImmutableBiMap<K, V> create(int n, Entry<K, V>[] entryArray) {
    ...
    for (int i = 0; i < n; i++) {
      Entry<K, V> e = RegularImmutableMap.makeImmutable(Objects.<Entry<K, V>>requireNonNull(entryArray[i]));
      entryArray[i] = e;
    ...
    ImmutableList<Entry<K, V>> entryList = ImmutableList.asImmutableList((Object[])entryArray, n);
...
/* Location:              C:\Users\Win10\Desktop\VoxilityCraftServer\Bungee\BungeeCord.jar!\com\google\common\collect\JdkBackedImmutableBiMap.class
