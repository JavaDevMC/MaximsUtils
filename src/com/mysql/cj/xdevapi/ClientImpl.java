...
public class ClientImpl implements Client, Protocol.ProtocolEventListener {
    ...
    if (pooling != null) {
      ...
      if (jsonVal != null)
        if (JsonLiteral.class.isAssignableFrom(jsonVal.getClass())) {
          ...
          if (pe != JsonLiteral.FALSE && pe != JsonLiteral.TRUE)
            throw new XDevAPIError(String.format("Client option '%s' does not support value '%s'.", new Object[] { ClientProperty.POOLING_ENABLED.getKeyName(), jsonVal
                    .toFormattedString() })); 
          ...
          if (JsonString.class.isAssignableFrom(jsonVal.getClass()))
            throw new XDevAPIError(String.format("Client option '%s' does not support value '%s'.", new Object[] { ClientProperty.POOLING_ENABLED.getKeyName(), ((JsonString)jsonVal)
                    .getString() })); 
          throw new XDevAPIError(String.format("Client option '%s' does not support value '%s'.", new Object[] { ClientProperty.POOLING_ENABLED.getKeyName(), jsonVal
                  .toFormattedString() }));
        }  
      ...
      if (jsonVal != null)
        if (JsonNumber.class.isAssignableFrom(jsonVal.getClass())) {
          ...
          if (JsonString.class.isAssignableFrom(jsonVal.getClass()))
            throw new XDevAPIError(String.format("Client option '%s' does not support value '%s'.", new Object[] { ClientProperty.POOLING_MAX_SIZE.getKeyName(), ((JsonString)jsonVal)
                    .getString() })); 
          throw new XDevAPIError(String.format("Client option '%s' does not support value '%s'.", new Object[] { ClientProperty.POOLING_MAX_SIZE.getKeyName(), jsonVal
                  .toFormattedString() }));
        }  
      ...
      if (jsonVal != null)
        if (JsonNumber.class.isAssignableFrom(jsonVal.getClass())) {
          ...
          if (JsonString.class.isAssignableFrom(jsonVal.getClass()))
            throw new XDevAPIError(String.format("Client option '%s' does not support value '%s'.", new Object[] { ClientProperty.POOLING_MAX_IDLE_TIME.getKeyName(), ((JsonString)jsonVal)
                    .getString() })); 
          throw new XDevAPIError(String.format("Client option '%s' does not support value '%s'.", new Object[] { ClientProperty.POOLING_MAX_IDLE_TIME.getKeyName(), jsonVal
                  .toFormattedString() }));
        }  
      ...
      if (jsonVal != null)
        if (JsonNumber.class.isAssignableFrom(jsonVal.getClass())) {
          ...
          if (JsonString.class.isAssignableFrom(jsonVal.getClass()))
            throw new XDevAPIError(String.format("Client option '%s' does not support value '%s'.", new Object[] { ClientProperty.POOLING_QUEUE_TIMEOUT.getKeyName(), ((JsonString)jsonVal)
                    .getString() })); 
          throw new XDevAPIError(String.format("Client option '%s' does not support value '%s'.", new Object[] { ClientProperty.POOLING_QUEUE_TIMEOUT.getKeyName(), jsonVal
                  .toFormattedString() }));
        }  
  ...
  private void validateAndInitializeClientProps(Properties clientProps) {
    ...
    List<String> clientPropsAsString = (List<String>)Stream.<ClientProperty>of(ClientProperty.values()).map(ClientProperty::getKeyName).collect(Collectors.toList());
...
/* Location:              C:\Users\Win10\Desktop\VoxilityCraftServer\Bungee\BungeeCord.jar!\com\mysql\cj\xdevapi\ClientImpl.class
