...
public class SchemaImpl implements Schema {
  ...
  public List<Collection> getCollections(String pattern) {
    Set<String> strTypes = (Set<String>)Arrays.<DbObjectType>stream(new DbObjectType[] { DbObjectType.COLLECTION }).map(Enum::toString).collect(Collectors.toSet());
  ...
  public List<Table> getTables(String pattern) {
    Set<String> strTypes = (Set<String>)Arrays.<DbObjectType>stream(new DbObjectType[] { DbObjectType.TABLE, DbObjectType.VIEW, DbObjectType.COLLECTION_VIEW }).map(Enum::toString).collect(Collectors.toSet());
    Predicate<Row> rowFiler = r -> strTypes.contains(r.getValue(1, this.svf));
    Function<Row, String> rowToName = r -> (String)r.getValue(0, this.svf);
    List<String> objectNames = (List<String>)this.mysqlxSession.query((Message)this.xbuilder.buildListObjects(this.name, pattern), rowFiler, rowToName, Collectors.toList());
...
/* Location:              C:\Users\Win10\Desktop\VoxilityCraftServer\Bungee\BungeeCord.jar!\com\mysql\cj\xdevapi\SchemaImpl.class
