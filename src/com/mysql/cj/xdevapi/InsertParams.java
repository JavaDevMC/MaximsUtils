...
public class InsertParams {
  ...
  public void setProjection(String[] projection) {
    this.projection = (List<MysqlxCrud.Column>)Arrays.<String>stream(projection).map(p -> (new ExprParser(p, true)).parseTableInsertField()).collect(Collectors.toList());
  }
  ...
  public void addRow(List<Object> row) {
    this.rows.add(MysqlxCrud.Insert.TypedRow.newBuilder().addAllField((Iterable)row.stream().map(f -> ExprUtil.argObjectToExpr(f, true)).collect(Collectors.toList())).build());
  }
...
/* Location:              C:\Users\Win10\Desktop\VoxilityCraftServer\Bungee\BungeeCord.jar!\com\mysql\cj\xdevapi\InsertParams.class
