...
public class ExprParser {
          ...
          switch (c) {
            ...
            case '`':
              ...
              try {
                ...
                for (; c != quoteChar || (i + 1 < this.string.length() && this.string.charAt(i + 1) == quoteChar); c = this.string.charAt(++i)) {
                  if (c == '\\' || c == quoteChar)
                    i++; 
  ...
  MysqlxExpr.Expr atomicExpr() {
    ...
    switch (t.type) {
      ...
      case LNUM_INT:
        return ExprUtil.buildLiteralScalar(Long.valueOf(t.value).longValue());
      case LNUM_DOUBLE:
        return ExprUtil.buildLiteralScalar(Double.valueOf(t.value).doubleValue());
      case TRUE:
      case FALSE:
...
/* Location:              C:\Users\Win10\Desktop\VoxilityCraftServer\Bungee\BungeeCord.jar!\com\mysql\cj\xdevapi\ExprParser.class
