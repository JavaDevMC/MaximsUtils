...
public class ClientPreparedStatement extends StatementImpl implements JdbcPreparedStatement {
  ...
  public ClientPreparedStatement(JdbcConnection conn, String sql, String db, ParseInfo cachedParseInfo) throws SQLException {
    ...
    try {
      ...
      ((PreparedQuery)this.query).setParseInfo((cachedParseInfo != null) ? cachedParseInfo : new ParseInfo(sql, (Session)this.session, this.charEncoding));
    } catch (CJException e) {
  ...
  public boolean execute() throws SQLException {
    try {
      synchronized (checkClosed().getConnectionMutex()) {
        ...
        rs = executeInternal(this.maxRows, sendPacket, createStreamingResultSet(), (getParseInfo().getFirstStmtChar() == 'S'), (ColumnDefinition)cachedMetadata, false);
  ...
  protected ClientPreparedStatement prepareBatchedInsertSQL(JdbcConnection localConn, int numBatches) throws SQLException {
    try {
      synchronized (checkClosed().getConnectionMutex()) {
        ClientPreparedStatement pstmt = new ClientPreparedStatement(localConn, "Rewritten batch of: " + ((PreparedQuery)this.query).getOriginalSql(), getCurrentDatabase(), getParseInfo().getParseInfoForBatch(numBatches));
    ...
    try {
      synchronized (checkClosed().getConnectionMutex()) {
        ...
        if (this.pstmtResultMetaData == null)
          try {
            mdStmt = new ClientPreparedStatement(this.connection, ((PreparedQuery)this.query).getOriginalSql(), getCurrentDatabase(), getParseInfo());
            mdStmt.setMaxRows(1);
  ...
  private void initializeFromParseInfo() throws SQLException {
    try {
      synchronized (checkClosed().getConnectionMutex()) {
        ...
        ((PreparedQuery)this.query).setQueryBindings((QueryBindings)new ClientPreparedQueryBindings(parameterCount, (Session)this.session));
        ((ClientPreparedQueryBindings)((ClientPreparedQuery)this.query).getQueryBindings()).setLoadDataQuery(getParseInfo().isLoadData());
        clearParameters();
      } 
      return;
    } catch (CJException cJException) {
  ...
  public void setTimestamp(int parameterIndex, Timestamp x, Calendar targetCalendar, int fractionalLength) throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      ((PreparedQuery)this.query).getQueryBindings().setTimestamp(getCoreParameterIndex(parameterIndex), x, targetCalendar, fractionalLength, MysqlType.TIMESTAMP);
    } 
  }
...
/* Location:              C:\Users\Win10\Desktop\VoxilityCraftServer\Bungee\BungeeCord.jar!\com\mysql\cj\jdbc\ClientPreparedStatement.class
