...
public class XDevApiConnectionUrl extends ConnectionUrl {
  ...
  public XDevApiConnectionUrl(ConnectionUrlParser connStrParser, Properties info) {
    ...
    for (HostInfo hi : this.hosts) {
      ...
      if (hasPriority)
        try {
          ...
          if (priorities.contains(Integer.valueOf(priority))) {
            this.hasDuplicatedPriorities = true;
            continue;
          } 
          priorities.add(Integer.valueOf(priority));
    ...
    if (hasPriority) {
      ...
      this.hosts.sort(
          Comparator.comparing(hi -> Integer.valueOf(Integer.parseInt((String)hi.getHostProperties().get(PropertyKey.PRIORITY.getKeyName())))).reversed());
    } 
  }
  ...
  protected void preprocessPerTypeHostProperties(Map<String, String> hostProps) {
    if (hostProps.containsKey(PropertyKey.ADDRESS.getKeyName())) {
      ...
      if (port.intValue() != -1 && !hostProps.containsKey(PropertyKey.PORT.getKeyName()))
  ...
  public List<HostInfo> getHostsList(HostsListView view) {
    if (this.prioritySorted) {
      if (this.hasDuplicatedPriorities) {
        Map<Integer, List<HostInfo>> hostsByPriority = (Map<Integer, List<HostInfo>>)this.hosts.stream().collect(Collectors.groupingBy(hi -> Integer.valueOf((String)hi.getHostProperties().get(PropertyKey.PRIORITY.getKeyName()))));
        this
          
          .hosts = (List)hostsByPriority.entrySet().stream().sorted(Comparator.comparing(Map.Entry::getKey).reversed()).map(Map.Entry::getValue).peek(Collections::shuffle).flatMap(Collection::stream).collect(Collectors.toList());
      } 
    } else {
...
/* Location:              C:\Users\Win10\Desktop\VoxilityCraftServer\Bungee\BungeeCord.jar!\com\mysql\cj\con\\url\XDevApiConnectionUrl.class
