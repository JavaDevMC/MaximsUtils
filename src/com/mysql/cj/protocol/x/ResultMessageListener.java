...
public class ResultMessageListener<R> implements MessageListener<XMessage> {
  ...
  public boolean processMessage(XMessage message) {
    Class<? extends GeneratedMessageV3> msgClass = (Class)message.getMessage().getClass();
    if (Mysqlx.Error.class.equals(msgClass)) {
      this.future.completeExceptionally((Throwable)new XProtocolError(Mysqlx.Error.class.cast(message.getMessage())));
    } else if (!this.messageToProtocolEntityFactory.containsKey(msgClass)) {
      this.future.completeExceptionally((Throwable)new WrongArgumentException("Unhandled msg class (" + msgClass + ") + msg=" + message.getMessage()));
    } else {
...
/* Location:              C:\Users\Win10\Desktop\VoxilityCraftServer\Bungee\BungeeCord.jar!\com\mysql\cj\protocol\x\ResultMessageListener.class
