...
public class XProtocol extends AbstractProtocol<XMessage> implements Protocol<XMessage> {
  ...
  public void negotiateSSLConnection() {
    ...
    try {
      ...
      this.reader = new SyncMessageReader(this.socketConnection.getMysqlInput(), (ProtocolEventHandler)this);
    } catch (IOException e) {
  ...
  public void negotiateCompression() {
    ...
    String[] compressionAlgorithmsOrder = (String[])Arrays.<String>stream(compressionAlgsOrder).sequential().filter(n -> (n != null && n.length() > 0)).map(String::toLowerCase).map(CompressionAlgorithm::getNormalizedAlgorithmName).toArray(x$0 -> new String[x$0]);
  ...
  public void afterHandshake() {
    if (this.compressionEnabled) {
      try {
        this
          .reader = new SyncMessageReader(new FullReadInputStream(new CompressionSplittedInputStream((InputStream)this.socketConnection.getMysqlInput(), new CompressorStreamsFactory(this.compressionAlgorithm))), (ProtocolEventHandler)this);
      } catch (IOException e) {
...
/* Location:              C:\Users\Win10\Desktop\VoxilityCraftServer\Bungee\BungeeCord.jar!\com\mysql\cj\protocol\x\XProtocol.class
