...
public class XServerCapabilities implements ServerCapabilities {
  ...
  public boolean getTls() {
    return hasCapability(KEY_TLS) ? ((MysqlxDatatypes.Any)this.capabilities.get(KEY_TLS)).getScalar().getVBool() : false;
  }
  ...
  public List<String> getAuthenticationMechanisms() {
    return (List<String>)((MysqlxDatatypes.Any)this.capabilities.get(KEY_AUTHENTICATION_MECHANISMS)).getArray().getValueList().stream()
  ...
  public Map<String, List<String>> getCompression() {
    if (hasCapability(KEY_COMPRESSION))
      return (Map<String, List<String>>)((MysqlxDatatypes.Any)this.capabilities.get(KEY_COMPRESSION)).getObj().getFldList().stream()
        .collect(Collectors.toMap(f -> f.getKey().toLowerCase(), f -> (List)f.getValue().getArray().getValueList().stream().map(()).collect(Collectors.toList()))); 
...
/* Location:              C:\Users\Win10\Desktop\VoxilityCraftServer\Bungee\BungeeCord.jar!\com\mysql\cj\protocol\x\XServerCapabilities.class
