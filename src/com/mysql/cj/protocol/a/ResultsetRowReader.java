...
public class ResultsetRowReader implements ProtocolEntityReader<ResultsetRow, NativePacketPayload> {
  ...
  public ResultsetRow read(ProtocolEntityFactory<ResultsetRow, NativePacketPayload> sf) throws IOException {
    ...
    rowPacket = (NativePacketPayload)this.protocol.getPacketReader().readMessage(rf.canReuseRowPacketForBufferRow() ? Optional.<NativePacketPayload>ofNullable(this.protocol.getReusablePacket()) : Optional.empty(), hdr);
    this.protocol.checkErrorMessage(rowPacket);
    rowPacket.setPosition(rowPacket.getPosition() - 1);
    if ((!this.protocol.getServerSession().isEOFDeprecated() && rowPacket.isEOFPacket()) || (this.protocol
      .getServerSession().isEOFDeprecated() && rowPacket.isResultSetOKPacket())) {
...
/* Location:              C:\Users\Win10\Desktop\VoxilityCraftServer\Bungee\BungeeCord.jar!\com\mysql\cj\protocol\a\ResultsetRowReader.class
