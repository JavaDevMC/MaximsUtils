...
public class TimeUtil {
  ...
  public static Boolean hasFractionalSeconds(Time t) {
    return Boolean.valueOf((t.getTime() % 1000L > 0L));
  }
  ...
  public static Object parseToDateTimeObject(String s, MysqlType targetMysqlType) throws IOException {
    ...
    if (TIME_LITERAL_WITH_DELIMITERS.matcher(s).matches())
      return LocalTime.parse(getCanonicalTime(s), (new DateTimeFormatterBuilder())
    ...
    if (TIME_LITERAL_SHORT6.matcher(s).matches())
      return LocalTime.parse(s, (new DateTimeFormatterBuilder())
    ...
    if (TIME_LITERAL_SHORT4.matcher(s).matches())
      return LocalTime.parse("00" + s, (new DateTimeFormatterBuilder())
          .appendPattern("HHmmss").appendFraction(ChronoField.NANO_OF_SECOND, 0, 9, true).toFormatter()); 
    if (TIME_LITERAL_SHORT2.matcher(s).matches())
      return LocalTime.parse("0000" + s, (new DateTimeFormatterBuilder())
          .appendPattern("HHmmss").appendFraction(ChronoField.NANO_OF_SECOND, 0, 9, true).toFormatter()); 
    if (DATETIME_LITERAL_SHORT14.matcher(s).matches())
      return LocalDateTime.parse(s, (new DateTimeFormatterBuilder())
    ...
    if (DATETIME_LITERAL_WITH_DELIMITERS.matcher(s).matches())
      return LocalDateTime.parse(getCanonicalDateTime(s), (new DateTimeFormatterBuilder())
  ...
  private static String getCanonicalDate(String s) {
    ...
    if (sa[0].length() == 2)
      sb.append((Integer.valueOf(sa[0]).intValue() > 69) ? "19" : "20"); 
    sb.append(sa[0]);
    sb.append("-");
    if (sa[1].length() == 1)
      sb.append("0"); 
...
/* Location:              C:\Users\Win10\Desktop\VoxilityCraftServer\Bungee\BungeeCord.jar!\com\mysql\c\\util\TimeUtil.class
